...
class Rbac {
  ...
  authorize (id, body) {
    ...
    if (isNaN(id)) {
      return Promise.reject(new TypeError('Invalid userId value: must be a number.'))
    }
    ...
    return this._opts
      ...
      .then((principalPermissions) => {
        const granted = ((type) => {
          switch (type) {
            case null: return Hoek.intersect(permissions, principalPermissions).length === permissions.length
            case 'OR': return Hoek.contain(permissions, principalPermissions)
            case 'AND': return Hoek.deepEqual(permissions, principalPermissions, { prototype: false })
  ...
  authorizeRemote (permission, headers) {
    ...
    return this._authorizeRemote([ permission ], headers)
  }
  ...
  authorizeRemoteOr (permissions, headers) {
    ...
    return this._authorizeRemote(permissions, headers, 'or')
  }
