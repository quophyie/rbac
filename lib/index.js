...
class Rbac {
   ...
   * @param {string} [reqType] - the namespace where the type is setup in the request, for the express middleware.
   */
  constructor (opts) {
   ...
   */
  authorize (userId, type, permission, opts) {
    ...
    if (principalOpts.remoteAuth) {
      return this._checkPermissionRemote(principalOpts.remoteAuth.url, principalOpts.remoteAuth.headers, userId, permission)
    } else {
      // It's a local permission validation, call the callback
      return principalOpts.checkPermission(userId, permission)
    }
  }
