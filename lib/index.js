...
class Rbac {
   ...
   * `function (user, permission)` and returning a Promise. If you're using the Expressjs middleware, `user` will be the
   * same as `req.user` and `permission` the string setup in the middleware. If `opts.remoteAuth` is not set, then this
   ...
   * @param {object|string|number} user - The user to be checked for permission.
   ...
   * @returns {Promise.<*>} - A promise resolving to the user allowed for the given permission. Depending on the used
   * method (local/remote) and on the callback/HTTP request's response, the promise might have different values.
   */
  allow (user, permission, opts) {
    ...
    if (opts.remoteAuth) {
      return this._checkPermissionRemote(opts.remoteAuth.url, opts.remoteAuth.headers, user, permission)
    } else {
      // It's a local permission validation, call the callback
      return opts.checkPermission(user, permission)
    }
  }
   ...
   * @returns {Promise.<*>} - A promise resolving to the user allowed for the given permission.
   * @private
   */
  _checkPermissionRemote (url, headers, user, permission) {
    const opts = {
      uri: url,
      method: 'POST',
      headers: headers,
