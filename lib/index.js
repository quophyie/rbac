...
class Rbac {
  ...
  authorize (id, body) {
    ...
    let checkType = (body.checkType) ? body.checkType.toUpperCase() : null
      ...
      .then((principalPermissions) => {
        const granted = ((type) => {
          switch (type) {
            ...
            case 'OR':
              return Hoek.contain(principalPermissions, permissions, { part: true })
            case 'AND':
              return Hoek.contain(principalPermissions, permissions)
            default:
              return false
          }
        })(checkType)
