...
class Rbac {
   ...
   * @param {string|null} checkType - The permissions check type to be applied
   * @returns {Promise.<*>} - A promise resolving to the principal being authorized for a specific permission.
   */
  authorize (id, permissions, checkType) {
    return Promise
      .all([])
      .then(() => (isNaN(id))
        ? Promise.reject(new TypeError('Invalid userId value: must be a number.'))
        : null)
      .then(() => (!Array.isArray(permissions))
        ? Promise.reject(new TypeError('Invalid permissions value: must be an array'))
        : null)
      .then(() => (!this._opts.getPermission)
        ? Promise.reject(new Error('Local authorization not configured.'))
        : null)
      .then(() => this._opts.getPermission(id))

      .then(principalPermissions =>
        (checkType === null) ? Hoek.intersect(permissions, principalPermissions, true) :
          (checkType === 'or') ? Hoek.contain(permissions, principalPermissions) :
            (checkType === 'and') ? permissions.filter(p => principalPermissions.indexOf(p) > -1)
              : false)
      .then(foundPermission => (foundPermission)
        ? Promise.resolve(foundPermission)
