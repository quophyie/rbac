  ...
  _authorizeMiddleware (permissions, checkType = null) {
    return (req, res, next) => {
      ...
      return Promise
        ...
        .then((user) => (checkType === 'OR') ? this._rbac.authorizeRemoteOr(permissions, headers) : user)
        .then((user) => (checkType === 'AND') ? this._rbac.authorizeRemoteAnd(permissions, headers) : user)
