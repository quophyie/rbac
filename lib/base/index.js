...
/**
 * A base implentation of Rbac System. This class drives the rbac system
 ...
 */
function RbacBase (rolesDalImpl, /* clientsDalImpl,*/ permsGroup, conjunction) {
  ...
  /* if (!(clientsDalImpl instanceof UsersDal)) {
    throw new TypeError('Parameter "clientsDalImpl" must be of Type "UsersDal"')
  } else {
    usersDal = clientsDalImpl
  } */
...
RbacBase.prototype.initialize = function () {
  rules = {}

  // Private methods
  // Creates rules permissions for a given role. Uses the rolesDal to obtain permissions for a given role
  /**
   @param {RolesDal} rolesDalImpl - An implentation of the RolesDal interface that is used to access roles and permissions from a datasource
   @param {string} roleName: The name of the role whose permissions are to be added
   @param {string} permsGroup - The permissionGroup that RbacBase must work on. It allows callers to define different groups of permissions. For example 'Credentials' group, 'UserAccess' group. If not
  the RbacBase will assume the 'DEFAULT' permission group.
   @param {boolean} conjunction: a boolean telling whether all permissions must by ANDed or ORed when testing validity to access resource
   @returns {Promise}
   */
   ...
   *
   * @param {string} ruleKey - the key of the rule to me updated in the rules object
   * @param {string } rulePermissions - An array of rule permissions to be applied
   * @param {boolean} conjunction: a boolean telling whether all permissions must by ANDed or ORed when testing validity to access resource. See @constructor for more detailed explanation
   * @returns Promise
   */
    ...
    return createRulePermissions(rolesDal, roleName, permissionsGroup, conjunction, ruleKey)
      .then(function (rulePermissions) {
        if (rules[ruleKey] === undefined || rules[ruleKey] === null) {
          return createAndAddRuleToRulesCollection(ruleKey, rulePermissions, conjunction)
        } else {
  ...
  /*
   @param {string} ruleKey - The key that is used to identify the rule in rules array. Must be of the form '[permissionGroup]:[roleName]' e.g. DEFAULT:testRole
   @param {array} rulePermissions - a string array of permissions for the given role e.g. ['read', 'write']
   @param {boolean} conjunction - a boolean telling whether all permissions must by ANDed or ORed when testing validity to access resource. See @constructor for more detailed explanation
   * @returns Promise
   */
  let createAndAddRuleToRulesCollection = (ruleKey, rulePermissions, conjunction) => {
    if (!check.string(ruleKey)) {
      throw new TypeError('Parameter "roleName" must be of Type "String"')
    }
    if (!check.array(rulePermissions)) {
...
/**
 * Permits access to a resource depending on whether the supplied user has one of the permissions in the permissionNames
 *
 * @param {primaryKey} memberId - The member id of the member to test,
 * @param {enum} memberType [optional] - The member type of the member to test
 * @param [array] permissionNames - an array of permission names to test against the given user
 * @param {string} permGroup - the permission group which the permision names in the permissionNames belong
 * @returns Promise
 */
RbacBase.prototype.permit = (memberId, memberType, permissionNames, permGroup) => {
  if (!permissionNames) {
  ...
  return new Promise((resolve, reject) => {
    let bExit = false
    let rulekey = ''
    rolesDal.findMemberRoles(memberId, memberType).then((roles) => {
...
/**
 * Returns a list of rules associated with this instance of RbacBase
 * @returns Promise
 */
...
/**
 * Deniew access to a resource depending on whether the supplied user has one of the permissions in the permissionNames
 *
 * @param {primaryKey} userId - The user id of the user to test
 * @param [array] permissionNames - an array of permission names to test against the given user
 * @param {string} permGroup - the permission group which the permision names in the permissionNames belong
 * @returns Promise
 */
