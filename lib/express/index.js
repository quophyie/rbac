...
const Express = {
  ...
  allow: (permissions, permissionsGroup) => {
    return (req, res, next) => {
      ...
      if (req.user && req.user.id) {
        rbacBase.permit(req.user.id, req.user.clientType, permissions, permissionsGroup)
          .then((bIsAllowed) => {
  ...
  deny: (permissions, permissionsGroup) => {
    ...
    return (req, res, next) => {
      if (req.user && req.user.id) {
        rbacBase.deny(req.user.id, req.user.clientType, permissions, permissionsGroup)
          .then((bIsDenied) => {
            // Set isProcessed to true to indicate that that the request has gone through the rbac pipeline
            req.body.c8rbac = {isProcessed: true}
            if (bIsDenied) {
